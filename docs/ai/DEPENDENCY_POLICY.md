...
Record in the implementation plan or change summary:
...
The repository uses `config/dependency-policy.conf` and `scripts/check-dependencies.sh`.
...
Add exact, case-sensitive package coordinates to `config/dependency-denylist.txt`, one per line:
...
For high-assurance projects, set `DEPENDENCY_ALLOWLIST_MODE=1` and populate `config/dependency-allowlist.txt`. All detected direct dependencies must then be listed. This is intentionally strict and normally unsuitable during early prototyping.
...
1. `scripts/dependency_policy.py` for local policy and lockfile checks.
